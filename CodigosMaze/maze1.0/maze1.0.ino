...
/*
victimasCol
  Se utiliza para detectar las victimas de colores con la pixy
*/
void victimasCol(){
  char buf[128];
  // Busca si hay colores en el rango
  int bloques = pixy.ccc.getBlocks();
  if (bloques) {
    if (pixy.ccc.numBlocks) {
      for (int i = 0; i < pixy.ccc.numBlocks; i++) {
        // Si detecta imprime el color de la victima
        if (pixy.ccc.blocks[i].m_signature == 1 ) {
          Serial.print("Roja");
        }
        else if (pixy.ccc.blocks[i].m_signature == 2 ) {
          Serial.print("Amarilla");
        }
...
void loop(){
  ...
  //estas variables estan definidas al inicio del codigo 
  //ab_serial=0;
  //ab_grabar=1;
  //imp=28;
  //LL=200;    
  //LD=60; 
  //esta impresion esta en el procedimiento aserie     
  //Serial.print("El contador: ");Serial.println(ciclo);
}
