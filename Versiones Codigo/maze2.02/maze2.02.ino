...
/* Variables de Duración de los Movimientos */
  int imp = 25;        
  int giros_I = 18;    // ciclos para iniciar el giro a la izquierda 
  int giros_ISF = 51;  // clclos para iniciar el giro en la inzquierda sin frente
  int giros_D = 17;    // ciclos para iniciar el giro a la derecha    
  int giros_u = 40;    // ciclos para iniciar el giro en U       
...
void pixyColores() {
  ...
  void leerPiso(){
    ...
    if((Rojo_Frec <= 30 && Rojo_Frec >= 10) and (Verde_Frec <= 30 && Verde_Frec >= 10) and (Azul_Frec <= 30 && Azul_Frec >= 10)){
      ...
      if((Rojo_Frec <= 65 && Rojo_Frec >= 45) and (Verde_Frec <= 65  && Verde_Frec >= 55) and (Azul_Frec <= 60 && Azul_Frec >= 50)){
        //color = "AZUL";
        if (contA <= 4){
          contA++;
          contB = 0;
  ...
  void izquierda(){
    ...
    if (ciclo<=giros_I){
      ...
      if (MDT > 66){   
        ...
        if(cont_Salir<=pasos_salir && MDT < 66){
          moverse(1,1);
          inercia="IZ";
          cont_Salir++;
        }else{
